...
- `SecretToolCredentialService`/`MacKeychainCredentialService.GetPassword()` gave up and returned `null` whenever the OS keychain cleanly reported "not found," without ever checking the encrypted file fallback — only an actual exception triggered the fallback lookup. This made the portable-mode password permanently unretrievable whenever `SetPassword` itself had to fall back to the file store (e.g. keychain access denied in a headless CI environment): every later command would fail with "Portable mode requires a password," even immediately after a successful-looking migration. Both now fall back to the file store on any non-success outcome, not just exceptions. Found via the CI hang above — fixing the timeout let execution reach far enough to expose this second, previously-hidden bug.
